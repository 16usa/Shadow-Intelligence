...
function entityProfitSort(a,b){
  const aKnown=a?.profitKnown!==false && Number.isFinite(Number(a?.profitUsd));
  const bKnown=b?.profitKnown!==false && Number.isFinite(Number(b?.profitUsd));

  if(aKnown!==bKnown)return aKnown?-1:1;
  if(!aKnown&&!bKnown)return String(a?.name||'').localeCompare(String(b?.name||''));

  const diff=Number(b.profitUsd)-Number(a.profitUsd);
...
function renderEntities(q=''){const query=(q||'').toLowerCase();const ranked=state.entities.slice().sort(entityProfitSort).map((e,index)=>({...e,entityRank:index+1}));const a=ranked.filter(e=>!query||[e.name,e.x_handle,e.notes].join(' ').toLowerCase().includes(query));$('#entitiesGrid').innerHTML=a.map(e=>`<article class="si-panel si-entity-card" data-entity="${e.id}" style="position:relative"><span class="si-entity-rank" aria-label="Rank ${e.entityRank}" title="Profit rank" style="position:absolute;top:18px;right:18px;font-size:14px;font-weight:700;line-height:1;letter-spacing:.01em;opacity:.52;pointer-events:none">#${e.entityRank}</span><div class="si-card-top">${avatar(e,'lg')}<div><h3>${esc(e.name)}</h3>${entityHandleHtml(e)}</div></div><div class="si-card-stats"><div class="si-card-stat"><strong class="${entityMetricClass(e,'profitUsd')}">${entityMetricMoney(e,'profitUsd')}</strong><small>Profit</small></div><div class="si-card-stat"><strong>${Number(e.performanceTokens||0)}</strong><small>Tokens</small></div><div class="si-card-stat"><strong class="${entityMetricClass(e,'avgProfitUsd')}">${entityMetricMoney(e,'avgProfitUsd')}</strong><small>Avg / Token</small></div></div></article>`).join('')||'<div class="guest-note">No entities found.</div>';$$('[data-entity]').forEach(x=>x.onclick=()=>openObject('entity',{id:x.dataset.entity}))}
