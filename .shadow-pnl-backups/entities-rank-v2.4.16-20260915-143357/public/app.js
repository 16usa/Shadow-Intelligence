...
function renderEntities(q=''){const query=(q||'').toLowerCase();const a=state.entities.slice().sort(entityProfitSort).filter(e=>!query||[e.name,e.x_handle,e.notes].join(' ').toLowerCase().includes(query));$('#entitiesGrid').innerHTML=a.map(e=>`<article class="si-panel si-entity-card" data-entity="${e.id}"><div class="si-card-top">${avatar(e,'lg')}<div><h3>${esc(e.name)}</h3>${entityHandleHtml(e)}</div></div><div class="si-card-stats"><div class="si-card-stat"><strong class="${entityMetricClass(e,'profitUsd')}">${entityMetricMoney(e,'profitUsd')}</strong><small>Profit</small></div><div class="si-card-stat"><strong>${Number(e.performanceTokens||0)}</strong><small>Tokens</small></div><div class="si-card-stat"><strong class="${entityMetricClass(e,'avgProfitUsd')}">${entityMetricMoney(e,'avgProfitUsd')}</strong><small>Avg / Token</small></div></div></article>`).join('')||'<div class="guest-note">No entities found.</div>';$$('[data-entity]').forEach(x=>x.onclick=()=>openObject('entity',{id:x.dataset.entity}))}
